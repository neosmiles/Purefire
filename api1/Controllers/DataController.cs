...
public class DataController : ControllerBase
{
    ...
    /// <summary>
    /// Test endpoint to verify organization header middleware functionality
    /// </summary>
    [AllowAnonymous]
    [HttpGet("test-organization-header")]
    public Task<IActionResult> TestOrganizationHeader([FromHeader(Name = "organizationId")] string? organiza)
    {
        // Get the organizationId header value (should be transformed by middleware)
        var organizationId = Request.Headers["organizationId"].FirstOrDefault();

        return Task.FromResult<IActionResult>(Ok(new
        {
            Message = "Organization header middleware test",
            OrganizationId = organizationId ?? "No organizationId header provided",
            HeaderExists = !string.IsNullOrEmpty(organizationId)
        }));
    }
}